...
const runCheckinGroupProcessor = async (consumerName) => {
  ...
  while (true) {
    ...
    if (response) {
      ...
      logger.debug(
        `${consumerName}: Updating user ${userKey} and location ${locationKey}.`
      );
      ...
      const ack = await redisClient.xack(
        likesStreamKey,
        CONSUMER_GROUP_NAME,
        checkinId
      );
      ...
      logger.info(
        `${consumerName}: ${
          ack === 1 ? "Acknowledged" : "Error acknowledging"
        } processing of checkin ${checkinId}.`
      );
      ...
      logger.info(`${consumerName}: waiting for more likes...`);
    }
  }
};
